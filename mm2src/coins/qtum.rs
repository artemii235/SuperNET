...
use crate::utxo::{ActualTxFee, AdditionalTxData, FeePolicy, UtxoArc, UtxoArcCommonOps, UtxoArcCommonOpsStandardImpl,
                  ...
                  UtxoMarketCoinOpsStandardImpl, UtxoMmCoinOpsStandardImpl, UtxoSwapOpsStandardImpl, withdraw_impl};
...
pub async fn qrc20_coin_from_conf_and_request(
    ...
    Ok(Qrc20Coin { inner, contract_address })
}
...
impl SwapOps for Qrc20Coin {
    ...
    fn validate_fee(&self, fee_tx: &TransactionEnum, fee_addr: &[u8], amount: &BigDecimal)
                    -> Box<dyn Future<Item=(), Error=String> + Send> {
        UtxoSwapOpsStandardImpl::validate_fee(self.inner.clone(), fee_tx, fee_addr, amount)
    }
    ...
    fn check_if_my_payment_sent(
        ...
        UtxoSwapOpsStandardImpl::check_if_my_payment_sent(
            self.inner.clone(), time_lock, other_pub, secret_hash, search_from_block)
    }
