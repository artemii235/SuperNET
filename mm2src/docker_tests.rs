...
mod docker_tests {
    ...
    fn match_and_trade_max() {
        ...
        unwrap! (block_on(mm_bob.wait_for_log (600., |log| log.contains (&format!("[swap uuid={}] Finished", uuid)))));
        unwrap! (block_on(mm_alice.wait_for_log (600., |log| log.contains (&format!("[swap uuid={}] Finished", uuid)))));
        unwrap!(block_on(mm_bob.stop()));
    ...
    #[test]
    fn test_swap_file_lock_prevents_double_swap_start_on_kick_start() {
        let (_, bob_priv_key) = generate_coin_with_random_privkey("MYCOIN", 1000);
        let (_, alice_priv_key) = generate_coin_with_random_privkey("MYCOIN1", 2000);
        let coins = json! ([
            {"coin":"MYCOIN","asset":"MYCOIN","txversion":4,"overwintered":1,"txfee":1000},
        ...
        // mm_bob using same DB dir that should kick start the swap, but fail due to file lock
        ...
        unwrap! (block_on (mm_bob_dup.wait_for_log (50.,
            |log| log.contains (&format!("Swap {} file lock is acquired by another process/thread, aborting", uuid))
        )));

        // mm_alice using same DB dir that should kick start the swap, but fail due to file lock
