...
use ser::{deserialize, serialized_list_size, serialized_list_size_with_flags, Serializable,
...
impl cmp::PartialEq for IndexedBlock {
    fn eq(&self, other: &Self) -> bool { self.header.hash == other.header.hash }
}
...
impl IndexedBlock {
    ...
    pub fn size_with_witness(&self) -> usize {
        ...
        let transactions = self.transactions.iter().map(|tx| &tx.raw).collect::<Vec<_>>();
        let txs_size =
            serialized_list_size_with_flags::<Transaction, &Transaction>(&transactions, SERIALIZE_TRANSACTION_WITNESS);
        header_size + txs_size
    }
