...
#[doc(hidden)] pub mod peers_tests;

#[cfg(feature = "native")] pub mod http_fallback;
#[cfg(feature = "native")]
...
use bytes::Bytes;
#[cfg(not(feature = "native"))] use common::executor::Timer;
#[cfg(not(feature = "native"))] use common::helperᶜ;
use common::log::TagParam;
use common::mm_ctx::{from_ctx, MmArc};
use common::{bits256, SlurpFut};
#[cfg(feature = "native")]
use common::{is_a_test_drill, slice_to_malloc, RaiiRm};
...
#[cfg(feature = "native")] use libc::{c_char, c_void};
use rand::{thread_rng, Rng, RngCore};
use serde::Serialize;
use serde_bencode::de::from_bytes as bdecode;
use serde_bencode::ser::to_bytes as bencode;
use serde_bytes::ByteBuf;
#[cfg(not(feature = "native"))] use serde_json::{self as json};
...
impl dugout_t {
    fn take_err(&mut self) -> Option<String> {
        if !self.err.is_null() {
            unsafe {
                let what = (if let Ok(msg) = CStr::from_ptr(self.err).to_str() {
                    msg
                } else {
                    "Non-unicode `what`"
                })
...
extern "C" {
    ...
    fn dht_alerts(
        dugout: *mut dugout_t,
        cb: extern "C" fn(dugout: *mut dugout_t, cbctx: *mut c_void, alert: *mut Alert),
        cbctx: *mut c_void,
    );
    ...
    fn dht_put(
        ...
        salt: *const u8,
        saltlen: i32,
        callback: extern "C" fn(*mut c_void, u64, *const u8, i32, *mut *mut u8, *mut i32, *mut i64),
        arg: *const c_void,
        arg2: u64,
    );
    // * `key` - The 32-byte seed which is given to `ed25519_create_keypair` in order to generate the key pair.
    ...
    P: Serialize,
{
    ...
    if extra_payload_size > 1400 {
        return ERR!("`extra_payload` is too large");
    }
    ...
    if let Some(err) = dugout.take_err() {
        return ERR!("lt_send_udp error: {}", err);
    }
    Ok(())
}
        ...
        match package.to {
            ...
            Either::Right(ref endpoint) => {
                for (ix, (payload, meta)) in (0..).zip(package.payloads.iter_mut()) {
                    if meta.pongs != 0 {
                        continue;
                    } // We don't retransmit the delivered discovery pings at present.
                    if meta.pings == u8::max_value() {
                        continue;
                    } // A natural limit on the number of retries.
            ...
            Either::Left((seed, ref send_handler)) => {
                ...
                if let Some(friend) = friends.get_mut(&seed) {
                    for (endpoint, _endpoint_meta) in friend.endpoints.iter_mut() {
                        for (payload, meta) in package.payloads.iter_mut() {
                            if meta.pongs != 0 {
                                continue;
                            } // TODO: Should retransmit ponged packets, but later and less often.
                            if meta.pings == u8::max_value() {
                                continue;
                            } // Reached a natural retransmission limit.
                            if limops > 33. {
                                continue;
                            }
                ...
                for (payload, meta) in package.payloads.iter_mut() {
                    ...
                    if meta.pings > 0 && now - package.scheduled_at < 1. {
                        continue;
                    }
                    ...
                    if now - meta.dht_put_invoked < 20. {
                        continue;
                    }
                    if limops > 10. {
                        continue;
                    }
                    if meta.dht_put_invoked > 0. && limops > 1. {
                        continue;
                    }
...
impl GetsEntry {
    ...
    fn drop_get(&mut self, frid: NonZeroU64) { self.futures.remove(&frid); }
}
...
fn chunk_to_gets(i_salt: &[u8], i_chunk: &[u8], our_public_id: &bits256, gets: &mut Gets) -> Result<u8, String> {
    if i_salt.len() < 2 {
        return ERR!("short ping salt");
    }
    ...
    if incoming_checksum != crc {
        return ERR!("bad ping chunk");
    }
    ...
    if chunk_idx > number_of_chunks.get() {
        return ERR!("ping chunk out of bounds");
    }
    gets.chunks
        .resize_with(number_of_chunks.get() as usize, Default::default);
...
fn hf_to_gets(ctx: &MmArc, gets: &mut Gets) -> Result<(), String> {
    ...
    if pctx.hf_last_poll_id.load(AtomicOrdering::Relaxed) != 0 {
        ...
        for (i_salt, (_from, chunk)) in hf_inbox.iter() {
            let subject_salt = &i_salt[0..i_salt.len() - 1];
            if !gets.contains_key(subject_salt) {
                continue;
            } // We're not currently getting that subject.
...
fn get_pieces_scheduler(
    ...
    let getsᵉ = match gets.entry(salt) {
        Entry::Vacant(getsᵉ) => {
            ...
            if option_env!("MM2_DHT_GET") != Some("f") {
                unsafe {
                    let our_public_id = unwrap!(ctx.public_id());
                    dht_get(
                        dugout,
                        our_public_id.bytes.as_ptr(),
                        our_public_id.bytes.len() as i32,
                        chunk_salt.as_ptr(),
                        chunk_salt.len() as i32,
                        pk.as_mut_ptr(),
                        pk.len() as i32,
                    )
                }
...
#[cfg(feature = "native")]
fn get_pieces_scheduler_en(ctx: &MmArc, dugout: &mut dugout_t, mut getsᵉ: OccupiedEntry<Salt, GetsEntry>) {
    ...
    for chunk in &getsᵉ.get().chunks {
        for &byte in unwrap!(chunk.payload.as_ref()) {
            buf.push(byte)
        }
    }
    if getsᵉ.get_mut().reassembled_at.is_none() {
        getsᵉ.get_mut().reassembled_at = Some(now)
    }
    ...
    let mut fetched = match pctx.recently_fetched.lock() {
        Ok(gets) => gets,
        Err(err) => {
            log!("get_pieces_scheduler] Can't lock the `PeersContext::recently_fetched`: "(err));
            return;
        },
    };
    match fetched.entry(getsᵉ.key().clone()) {
        Entry::Vacant(ve) => {
            ve.insert((now_float(), buf));
        },
        Entry::Occupied(oe) => *oe.into_mut() = (now_float(), buf),
    }
    for task in getsᵉ.get().futures.values() {
        task.notify()
    }
}
...
impl From<DhtDelayed> for DhtBootStatus {
    fn from(t: DhtDelayed) -> DhtBootStatus { DhtBootStatus::Delayed(t) }
}

impl From<DhtBootstrapping> for DhtBootStatus {
    fn from(t: DhtBootstrapping) -> DhtBootStatus { DhtBootStatus::Bootstrapping(t) }
}

impl From<DhtBootstrapped> for DhtBootStatus {
    fn from(t: DhtBootstrapped) -> DhtBootStatus { DhtBootStatus::Bootstrapped(t) }
}
...
fn incoming_ping(cbctx: &mut CbCtx, pkt: &[u8], ip: &[u8], port: u16) -> Result<(), String> {
    if option_env!("MM2_INCOMING_PING") == Some("f") {
        return Ok(());
    }
    let ping = match bdecode::<Ping<MmPayload>>(pkt) {
        Ok(p) => p,
        Err(_) => return Ok(()),
    };
    ...
    if from.len() != 32 {
        return ERR!("Wrong `from` length in a ping: {}", from.len());
    }
    let from = bits256 {
...
fn peers_thread(ctx: MmArc, _netid: u16, preferred_port: u16, read_only: bool, delay_dht: f64) {
    if let Err(err) = ctx.log.register_my_thread() {
        log!((err))
    }
    ...
    if !dht_state.is_empty() {
        ...
        // TODO: User-friendly log message (`LogState::log`).
        if let Some(err) = dugout.take_err() {
            log! ("dht_load_state (" [dht_stateᵖ] ") error: " (err))
        }
    }
    ...
    loop {
        extern "C" fn cb(_dugout: *mut dugout_t, cbctx: *mut c_void, alert: *mut Alert) {
            ...
            match rc.cmp(&0) {
                Ordering::Greater => {
                    ...
                    if idx == 0 {
                        return;
                    } // `idx` can't be 0, ergo the received payload is garbage.
                      //pintln! ("chunk " (idx) ", dht_mutable_item_alert, " [=rc] ' ' [=seq] ' ' [=auth]);

                    let payload: ByteBuf = if bencoded == b"0:" {
                        ByteBuf::new()
                    } else {
                        match serde_bencode::de::from_bytes(bencoded) {
                            Ok(payload) => payload,
                            Err(err) => {
                                log!("peers_thread] Can not decode the received payload: "(err));
                                return;
                            },
                        }
                    };
                    ...
                    let gets = match cbctx
                        .gets
                        .iter_mut()
                        .find(|en| en.0[..] == salt[..] && en.1.pk == Some(keybuf))
                    {
                        Some(gets_entry) => gets_entry.1,
                        None => return,
                    };
                    if idx > gets.chunks.len() {
                        return;
                    }
                    ...
                    if payload.len() < 5 {
                        return;
                    } // A chunk without a checksum and at least a single byte of payload is gibberish.
                    let incoming_checksum = match (&payload[payload.len() - 4..]).read_u32::<BigEndian>() {
                        Ok(c) => c,
                        Err(_err) => return,
                    };
                    for _ in 0..4 {
            ...
            if unsafe { is_dht_bootstrap_alert(alert) } {
                if let Some(status) = cbctx.ctx.log.claim_status(BOOTSTRAP_STATUS) {
                    status.append(" Done.")
                }
        ...
        match pctx.cmd_rx.recv_timeout(Duration::from_millis(100)) {
            ...
            Ok(LtCommand::DropGet { salt, frid }) => {
                if let Some(en) = gets.get_mut(&salt) {
                    en.drop_get(frid)
                }
        ...
        if let Err(err) = transmit(&mut dugout, &ctx, &hf_addr) {
            log!("transmit error: "(err))
        }
        ...
        for salt in get_salts {
            let gets_oe = if let Entry::Occupied(oe) = gets.entry(salt) {
                oe
            } else {
                panic!()
            };
...
pub async fn initialize(ctx: &MmArc, netid: u16, preferred_port: u16) -> Result<(), String> {
    ...
    let delay_dht = if ctx.conf["dht"].as_str() == Some("on") {
        0.
    } else if drill {
        33.
    } else {
    ...
    let pctx = try_s!(PeersContext::from_ctx(&ctx));
    *try_s!(pctx.peers_thread.lock()) = Some(try_s!(thread::Builder::new().name("peers".into()).spawn({
        let ctx = ctx.clone();
        ...
        Box::new(move || -> Result<(), String> {
            ...
            if let Ok(mut peers_thread) = pctx.peers_thread.lock() {
                if let Some(peers_thread) = peers_thread.take() {
                    let join_status = ctx
                        .log
                        .status(&[&"dht-stop"], "Waiting for the peers_thread to stop...");
...
pub async fn initialize(ctx: &MmArc, netid: u16, preferred_port: u16) -> Result<(), String> {
    ...
    try_s!(
        helperᶜ(
            "peers_initialize",
            try_s!(bencode(&ToPeersInitialize {
...
lazy_static! {
    static ref SEND_HANDLERS: Mutex<HashMap<u64, Arc<SendHandler>>> = Mutex::new(HashMap::new());
}
...
pub unsafe extern "C" fn peers_drop_send_handler(shp1: i32, shp2: i32) {
...
pub async fn send(
    ...
    let fallback = match option_env!("MM2_FALLBACK") {
        Some(n) => try_s!(n.parse()),
        None => fallback,
    };
...
impl Future for RecvFuture {
    ...
    fn poll(&mut self) -> Poll<Vec<u8>, String> {
        if self.frid.is_none() {
            let frid = loop {
                match NonZeroU64::new(thread_rng().gen()) {
                    Some(nz) => break nz,
                    None => continue,
                }
            };
            ...
            if let Err(err) = self.pctx.cmd_tx.send(get) {
...
impl Drop for RecvFuture {
    fn drop(&mut self) {
        if let Some(frid) = self.frid {
            if let Err(err) = self.pctx.cmd_tx.send(LtCommand::DropGet {
                salt: self.salt.clone(),
                frid,
            }) {
                log!("!send: "(err))
...
#[cfg(feature = "native")]
pub async fn recv(ctx: MmArc, subject: Vec<u8>, fallback: u8, validator: FixedValidator) -> Result<Vec<u8>, String> {
    let fallback = match option_env!("MM2_FALLBACK") {
        Some(n) => try_s!(n.parse()),
        None => fallback,
    };
...
#[cfg(not(feature = "native"))]
#[no_mangle]
pub extern "C" fn peers_check() -> i32 { 1 }
