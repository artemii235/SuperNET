...
pub async fn run_taker_swap(mut swap: TakerSwap, initial_command: Option<TakerSwapCommand>) {
    ...
    loop {
        ...
        for event in events {
            ...
            #[cfg(feature = "native")] unwrap!(save_my_taker_swap_event(&ctx, &uuid, to_save));
...
impl TakerSwap {
    ...
    async fn wait_for_taker_payment_spend(&self) -> Result<(Option<TakerSwapCommand>, Vec<TakerSwapEvent>), String> {
        ...
        let tx = match self.taker_coin.wait_for_tx_spend(&self.taker_payment.clone().unwrap().tx_hex, self.data.taker_payment_lock, self.data.taker_coin_start_block).compat().await {
            Ok(t) => t,
            Err(e) => return Ok((
                Some(TakerSwapCommand::RefundTakerPayment),
